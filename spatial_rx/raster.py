...
(default = ``DEFAULT_WINDOW_RADIUS`` = 10 world units / µm). That softens hard
...
def default_bin_size(
    ...
    Fixed at ``DEFAULT_BIN_SIZE`` (5). ``median_nn`` / ``point_size`` are
...
def default_window_radius(bin_size: float | None = None) -> float:
    ...
    proportional); otherwise ``DEFAULT_WINDOW_RADIUS`` (10).
    """
