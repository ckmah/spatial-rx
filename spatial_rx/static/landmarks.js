...
/** Fabio Crameri lipari via cmcrameri — low dark navy → high cream. */
const LIPARI_RGB = [
  [3, 19, 38],
  [4, 20, 40],
  [4, 22, 41],
...
function sampleLipari(t) {
  const u = Math.max(0, Math.min(1, t));
  const n = LIPARI_RGB.length;
  const x = u * (n - 1);
  const i0 = Math.floor(x);
  const i1 = Math.min(n - 1, i0 + 1);
  const f = x - i0;
  const a = LIPARI_RGB[i0];
  const b = LIPARI_RGB[i1];
  return [
...
function lipariCssGradient(dir = "to top") {
  // Sparse stops for legend CSS (full LUT used when painting bins).
  const stops = [0, 0.2, 0.4, 0.6, 0.8, 1].map((t) => {
    const [r, g, b] = sampleLipari(t);
...
const RASTER_QUERY_STROKE = "#fdf4da";
...
export function mountEngine({ model, host }) {
  ...
  function updatePointLegend() {
    ...
    if (isRasterMode()) {
      ...
      bar.style.background = lipariCssGradient("to top");
