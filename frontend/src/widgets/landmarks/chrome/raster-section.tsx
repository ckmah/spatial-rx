import {
  Tabs,
  TabsContent,
  TabsList,
  TabsTrigger,
} from "@/components/ui/tabs";

import type { LandmarksModel } from "../use-landmarks-model";
import {
  FIELD_CAPTION,
  META_LINE,
  PILL_TABS_LIST,
  PILL_TABS_TRIGGER,
} from "./sections";

/** Diverging similarity legend mid-stop at 0.5 (matches engine). */
const SIMILARITY_LEGEND =
  "linear-gradient(to right, #3b82f6 0%, #e8e8e8 50%, #ff0099 100%)";

/**
 * Geometry control (points vs bins) as Soft Float pill Tabs.
 * Similarity scale only in Bins; status when computing, errored, or pinned.
 */
export function RasterSection({ lm }: { lm: LandmarksModel }) {
  const view = lm.render_mode || "points";
  const status = lm.raster_status || "";
  const pinned = lm.raster_query_bin >= 0;

  let statusText: string | null = null;
  if (status === "computing") {
    statusText = "Computing bins…";
  } else if (status.startsWith("error:")) {
    statusText = status;
  } else if (pinned) {
    statusText = `Pinned bin ${lm.raster_query_bin} · Esc clears`;
  }

  return (
    <Tabs
      value={view}
      onValueChange={(v) => {
        if (v) lm.setRenderMode(v);
      }}
      className="flex flex-col gap-1.5"
      data-testid="raster-section"
    >
      <div className="flex flex-col gap-1">
        <p className={FIELD_CAPTION}>View</p>
        <TabsList className={PILL_TABS_LIST} aria-label="View">
          <TabsTrigger value="points" className={PILL_TABS_TRIGGER}>
            Points
          </TabsTrigger>
          <TabsTrigger value="raster" className={PILL_TABS_TRIGGER}>
            Bins
          </TabsTrigger>
        </TabsList>
      </div>

      <TabsContent
        value="raster"
        className="m-0 flex flex-col gap-1"
        data-testid="raster-legend"
      >
        <div
          className="h-2 w-full rounded-sm"
          style={{ background: SIMILARITY_LEGEND }}
          role="img"
          aria-label={
            pinned ? "Similarity to pinned bin" : "Similarity to hovered bin"
          }
          title={
            pinned
              ? "Similarity to pinned bin"
              : "Hover a bin to scrub similarity; click to pin"
          }
        />
        <div className="flex justify-between text-[10px] text-foreground/65">
          <span>Unlike</span>
          <span>Alike</span>
        </div>
        {statusText ? (
          <p className={META_LINE} data-testid="raster-status">
            {statusText}
          </p>
        ) : (
          <span className="sr-only" data-testid="raster-status">
            Ready
          </span>
        )}
      </TabsContent>
    </Tabs>
  );
}
