...
def test_default_bin_size_fixed_microns():
    ...
    assert default_bin_size(2.0) == pytest.approx(5.0)
    assert default_bin_size(None, point_size=0.5) == pytest.approx(5.0)
    assert default_bin_size(None) == pytest.approx(5.0)
    from spatial_rx.raster import default_window_radius

    assert default_window_radius(5.0) == pytest.approx(10.0)
    assert default_window_radius(None) == pytest.approx(10.0)
